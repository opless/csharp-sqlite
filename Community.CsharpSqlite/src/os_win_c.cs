...
dwFlagsAndAttributes |= FileOptions.RandomAccess; // FILE_FLAG_RANDOM_ACCESS;
 ...
 fs = new IsolatedStorageFileStream(zConverted, dwCreationDisposition, dwDesiredAccess, dwShareMode, IsolatedStorageFile.GetUserStoreForApplication());
            ...
            OSTRACE("OPEN %d (%s)\n", fs.GetHashCode(), fs.Name);
						...
						Console.Write ("Opening: {0}\nException: {1}", zConverted, e);
#if SQLITE_WINRT
    ...
    */
      ...
      if ( SimulateIOError() )
        return SQLITE_IOERR_DELETE;
			...
			//}
			if (isNT ()) {
				do {        //  DeleteFileW(zConverted);//}while(   (   ((rc = GetFileAttributesW(zConverted)) != INVALID_FILE_ATTRIBUTES)//           || ((error = GetLastError()) == ERROR_ACCESS_DENIED))//       && (++cnt < MX_DELETION_ATTEMPTS)//       && (Sleep(100), 1) );
#if SQLITE_WINRT
...
Debug.Assert(winSysInfo.dwAllocationGranularity > 0);
		...
		{
			#if !(SQLITE_SILVERLIGHT || WINDOWS_MOBILE || SQLITE_WINRT || NO_LOCK)
			[DllImport ("kernel32.dll")]
